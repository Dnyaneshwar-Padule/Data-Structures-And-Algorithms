...
void rotate_right_by_n_places(int*, int, int);
...
int main(){
    ...
    rotate_right_by_n_places(arr, len, d);
...
void rotate_right_by_n_places(int * arr, int len, int d){
    if (d < 1 || d >= len) return;
    d = d % len;
    reverse_array(arr, len - d  , len - 1);
    reverse_array(arr, 0, len - d - 1);
